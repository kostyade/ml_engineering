"""Train the model on selected data batches with W&B tracking."""
...
def train_model(
    ...
    """Train the model, log per-epoch metrics to W&B, save best checkpoint and loss plot."""
    model.to(device)
    ...
    # Log summary metric and image
    wandb.summary["best_val_loss"] = best_val_loss
    wandb.log({"loss_plot": wandb.Image(str(plot_path))})

    # Log model as W&B Artifact
    artifact = wandb.Artifact(
        name=f"model-{wandb.run.name}".replace("/", "_").replace(" ", "_"),
        type="model",
        description=f"Best CIFAR-10 SimpleCNN (val_loss={best_val_loss:.4f})",
    )
...
def run_training(config: Dict[str, Any]) -> None:
    """Run training for a single config inside an active W&B run."""
